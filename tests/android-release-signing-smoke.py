...
require(BUILD, 'versionCode = 3', 'android/app/build.gradle.kts')
require(BUILD, 'versionName = "0.1.0-alpha.3"', 'android/app/build.gradle.kts')
...
require(WORKFLOW, 'RELEASE_TAG: android-v0.1.0-alpha.3', '.github/workflows/android-release.yml')
...
# A future accidental version drift should fail loudly rather than silently publishing under alpha.3.
version_match = re.search(r'versionName\s*=\s*"([^"]+)"', BUILD)
if not version_match or version_match.group(1) != '0.1.0-alpha.3':
    raise AssertionError('release version drifted from 0.1.0-alpha.3')
