(() => {
  ...
  async function forwardDiagnostic(event) {
    ...
    if (!REPEATABLE_DIAGNOSTIC_EVENTS.has(event) && forwardedDiagnostics.has(event)) return;
    if (!REPEATABLE_DIAGNOSTIC_EVENTS.has(event)) forwardedDiagnostics.add(event);
    await sendNative({ type: "diagnostic", event });
  }
  ...
  function toCaptureBytes(body, expectedByteLength = null) {
    if (expectedByteLength !== null && (!Number.isSafeInteger(expectedByteLength) || expectedByteLength < 0)) return null;
    try {
      ...
      if (expectedByteLength !== null && bytes.byteLength !== expectedByteLength) return null;
      return bytes;
    } catch (_) {
      return null;
    }
  ...
  async function forwardStreamChunk(packet) {
    ...
    try {
      ...
      await sendNative({ type: "capture_chunk", captureId: packet.captureId, sequence: packet.sequence, base64 });
  ...
  async function forwardStreamEnd(packet) {
    if (typeof packet.captureId !== "string" || !Number.isSafeInteger(packet.byteLength) || packet.byteLength < 0) {
      await forwardDiagnostic("capture_metadata_rejected");
      return;
    }
    try {
      await sendNative({ type: "capture_end", captureId: packet.captureId, byteLength: packet.byteLength });
  ...
  async function forwardStreamAbort(packet) {
    if (typeof packet.captureId !== "string") {
      await forwardDiagnostic("capture_metadata_rejected");
      return;
    }
    try {
      await sendNative({ type: "capture_abort", captureId: packet.captureId });
      acknowledge(packet.captureId, "abort");
    } catch (error) {
      try { await forwardDiagnostic("capture_forward_failed"); } catch (_) {}
      throw error;
    }
  ...
  async function forwardCapture(packet) {
    ...
    try {
      ...
      for (let offset = 0; offset < bytes.length; offset += CHUNK_BYTES) {
        ...
        await forwardDiagnostic("chunk_encode_complete");
        await forwardDiagnostic("chunk_send_started");
        await sendNative({ type: "capture_chunk", captureId: metadata.captureId, sequence, base64 });
        await forwardDiagnostic("chunk_send_complete");
        sequence += 1;
      }
      await sendNative({ type: "capture_end", captureId: metadata.captureId, byteLength: metadata.byteLength });
      await forwardDiagnostic("capture_end_sent");
    } catch (error) {
      try { await sendNative({ type: "capture_abort", captureId: metadata.captureId }); } catch (_) {}
      try { await forwardDiagnostic("capture_forward_failed"); } catch (_) {}
      throw error;
    }
  }
  ...
  window.addEventListener("message", event => {
    ...
    if (event.source !== window || !packet || packet.source !== SOURCE) return;
    ...
    } else if (packet.kind === "capture_abort") {
      forwarding = forwarding.then(() => forwardStreamAbort(packet));
